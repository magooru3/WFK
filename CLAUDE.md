...
**The win modal scores the round and can share it.** `showEndModal(won)` fills two pills (`🎯 N tries`, `💡 N hints`) from `winTries` and `hintLevel`, and both the score row and the "Share with Grandma" button are `hidden` unless `won` — `.modal .score[hidden]` needs its own rule because `display: flex` otherwise beats the attribute. `winTries` is captured at the winning guess (`guessedRow + 1`), not read off `currentRow` in the modal, since `currentRow` has already advanced by then.

Sharing builds a text blob (title, "I got WORD in N tries with N hints", the emoji grid from `guessResults`, the Pages URL) and hands it to `navigator.share`, which is what opens iOS's share sheet with Messages in it. That call must happen inside the click gesture — don't `await` anything before it or iOS drops the sheet. `AbortError` means the user dismissed the sheet and is swallowed. Browsers without `navigator.share` fall back to the clipboard, then to an `sms:` deep link (`&body=` on Apple, `?body=` elsewhere). `guessResults` accumulates one `evaluateGuess` array per accepted guess purely to draw that grid.
